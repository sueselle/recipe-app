  ...
  if(showConfirmationModal) {
    return (
      <div className='recipe-details'>
        <ConfirmationModal message="Are you sure? You can't get it back once it's deleted!" onCancel={() => setShowConfirmationModal(false)} onConfirm = {() => handleDeleteRecipe(selectedRecipe.id)} />
      </div>
    );
  }
  return (
    <div className='recipe-details'>
      {editing ? (
        <EditRecipeForm selectedRecipe={selectedRecipe} onUpdateForm={onUpdateForm} handleCancel={handleCancel} handleUpdateRecipe={handleUpdateRecipe} />
      ) : (
      <article>
        <header>
          <figure>
